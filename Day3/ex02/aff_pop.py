...
def main():
    ...
    (or a second country specified in argv[2]), clips years to 1800-2050,
    formats the y-axis automatically, and displays a Seaborn line plot with the
    legend inside the plot.
    """
    path = ("population_total.csv" if len(argv) <= 1 else argv[1])
    campus = "France"
    country = ("South Korea" if len(argv) <= 2 else argv[2])
    dataset = load(path)
